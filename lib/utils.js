...
import { clamp } from "./calculateUtils";
...
export function moveElement(
  ...
  if (!collisions.length && oldDragItem && oldDragItem?.w !== l?.w) {
    l.w = oldDragItem.w;
  }
  ...
  for (let i = 0, len = collisions.length; i < len; i++) {
    ...
    if (collision.static) {
      ...
      const offset = collision.w + collision.x;
      const availableWidth = cols - offset;

      if (
        ox &&
        l.minW &&
        availableWidth < l.w &&
        availableWidth >= l.minW &&
        l.y === collision.y &&
